...
def invite_received_view(request):
    profile = Profile.objects.get(user=request.user)
    qs = Relationship.objects.invitations_receive(profile)
...
def accept_invitation(request):
    ...
    return redirect('profiles:my-invites-veiw')
...
def invites_profiles_list_view(request):
    ...
    is_empty = True if len(qs) == 0 else False
    context = {
        'is_empty':is_empty,
        'qs': qs,
    }
...
class ProfileDetailView( LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["len_posts"] = True if len(
            self.get_object().get_all_authors_post()) > 0 else False
        return context
...
def send_invitation(request):
    ...
    return redirect('profiles:my-profile-veiw')
